...
//          LoadParamsFromEEPROM(false);
    ...
    if (i != -1){  // have a request to set park angle Y
      ...
      if (( tv.dyParkWind < ANG_ABS_MIN_EW) || ( tv.dyParkWind > ANG_ABS_MAX_EW )){
        tv.dyParkWind = constrain(tv.dyParkWind ,ANG_ABS_MIN_EW ,ANG_ABS_MAX_EW ) ; // set to default
      }
    }        
    i = String(server.argName(j)).indexOf("parwx");
    if (i != -1){  // have a request to set park angle X
      tv.dxParkWind = String(server.arg(j)).toFloat() ;
      if (( tv.dxParkWind < ANG_ABS_MIN_NS) || ( tv.dxParkWind > ANG_ABS_MAX_NS )){
        tv.dxParkWind = constrain(tv.dxParkWind ,ANG_ABS_MIN_NS ,ANG_ABS_MAX_NS )   ; // set to default
      }
    }        
  ...
  if (String(server.uri()).indexOf("sensor")>0) {  // setup of the node
    ...
    server.sendContent(F("<tr><th>X</th><th>Y</th><th>Z</th><th>.</th></tr>")) ; 
    
    server.sendContent(F("<tr><td>Gyro</td><td align=center>")) ;
    server.sendContent(String(tv.xRoll)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(tv.yRoll)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(tv.zRoll)) ; 
    server.sendContent(F("</td><td align=center>(Rad/s)</td></tr>")) ;
    ...
    server.sendContent(F("<tr><td>Magnetometer Actual</td><td align=center>")) ;
    ...
    server.sendContent(String(running_min.x)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(running_min.y)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(running_min.z)) ; 
    server.sendContent(F("</td><td align=center>.</td></tr>")) ;

    server.sendContent(F("<tr><td>Running Max Value</td><td align=center>")) ;
    server.sendContent(String(running_max.x)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(running_max.y)) ; 
    server.sendContent(F("</td><td align=center>")) ;
    server.sendContent(String(running_max.z)) ; 
    server.sendContent(F("</td><td align=center>.</td></tr>")) ;
...
void SendHTTPHeader(){
  ...
  server.sendContent("</head><body><html><center><h2>Solar Tracker " + String(Toleo) + " </h2>");  
}
